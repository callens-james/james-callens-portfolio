...
###########################################################################
# rearrange.rFUNC <- function(funcpath) ({ setwd(funcpath) file <- read.csv(full_data_csvname) # Installs packages if not already installed list.of.packages <- c("tidyverse", "data.table") new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[, "Package"])] if (length(new.packages)) { install.packages(new.packages)} # Read Shiny csv df <- file # Remove binned and unneeded columns if("comments" %in% colnames(df)) { df <- df[, c("subject", "e.type", "group", "boxnum", "edate", "weight", "fr", "active", "inactive", "reward", "beam1", "beam2", "beam3", "beam4", "latency.to.first.active", "latency.to.first.inactive", "comments")] } else { df <- df[, c("subject", "e.type", "group", "boxnum", "edate", "weight", "fr", "active", "inactive", "reward", "beam1", "beam2", "beam3", "beam4", "latency.to.first.active", "latency.to.first.inactive")]} # Reorder data and remove unneeded column df <- df[order(df$edate, df$subject),] # Fix row names rownames(df) <- NULL # Adjust box comments phrasing if("comments" %in% colnames(df)) { df$comments <- gsub(".*:", "", df$comments)} # Get sum of beam breaks and rearrange/delete columns df$beam_breaks <- 0 for (c in 1:nrow(df)) { df[c, "beam_breaks"] <- df[c, "beam1"] + df[c, "beam2"] + df[c, "beam3"] + df[c, "beam4"]} # Remove individual beam break columns if("comments" %in% colnames(df)) { df <- df[, c("subject", "e.type", "group", "boxnum", "edate", "weight", "fr", "active", "inactive", "reward", "latency.to.first.active", "latency.to.first.inactive", "comments", "beam_breaks")] } else {df <- df[, c("subject", "e.type", "group", "boxnum", "edate", "weight", "fr", "active", "inactive", "reward", "latency.to.first.active", "latency.to.first.inactive", "beam_breaks")]} # Get session numbers from the date column df <- transform(df,session = as.numeric(factor(edate))) # Remove edate and e.type columns if("comments" %in% colnames(df)) { df <- df[, c("subject", "group", "boxnum", "weight", "fr", "active", "inactive", "reward", "latency.to.first.active", "latency.to.first.inactive", "comments", "beam_breaks", "session")] } else {df <- df[, c("subject", "group", "boxnum", "weight", "fr", "active", "inactive", "reward", "latency.to.first.active", "latency.to.first.inactive", "beam_breaks", "session")]} # Translate comment numbers to words library(data.table) if("comments" %in% colnames(df)) { for (c in 1:nrow(df)) { if (df[c, "comments"] %like% 00) {df[c, "comments"] = gsub("00", "misc", df[c, "comments"])} if (df[c, "comments"] %like% 01) {df[c, "comments"] = gsub("01", "exclude", df[c, "comments"])} if (df[c, "comments"] %like% 02) {df[c, "comments"] = gsub("02", "leak", df[c, "comments"])} if (df[c, "comments"] %like% 03) {df[c, "comments"] = gsub("03", "syringe", df[c, "comments"])} if (df[c, "comments"] %like% 04) {df[c, "comments"] = gsub("04", "wounded", df[c, "comments"])} if (df[c, "comments"] %like% 05) {df[c, "comments"] = gsub("05", "sick", df[c, "comments"])} if (df[c, "comments"] %like% 06) {df[c, "comments"] = gsub("06", "chewed", df[c, "comments"])} if (df[c, "comments"] %like% 07) {df[c, "comments"] = gsub("07", "cath issue", df[c, "comments"])} if (df[c, "comments"] %like% 08) {df[c, "comments"] = gsub("08", "hardware issue", df[c, "comments"])} if (df[c, "comments"] %like% 09) {df[c, "comments"] = gsub("09", "fire alarm", df[c, "comments"])} if (df[c, "comments"] %like% 10) {df[c, "comments"] = gsub("10", "escaped", df[c, "comments"])} if (df[c, "comments"] %like% 11) {df[c, "comments"] = gsub("11", "death", df[c, "comments"])} if (df[c, "comments"] %like% 12) {df[c, "comments"] = gsub("12", "software issue", df[c, "comments"])} if (df[c, "comments"] %like% 13) {df[c, "comments"] = gsub("13", "shielded", df[c, "comments"])} if (df[c, "comments"] %like% 14) {df[c, "comments"] = gsub("14", "autoshaped", df[c, "comments"])} if (df[c, "comments"] %like% 15) {df[c, "comments"] = gsub("15", "off tubing", df[c, "comments"])} } } head(df) # Make Beam Break rows to NA if the number is below 100 # This is a fail safe in case the nose poke holes register as beam breaks for (c in 1:nrow(df)) { if (df[c, "beam_breaks"] < 100) {df[c, "beam_breaks"] = NA} } # Tranform data set from long to wide format by session number library(tidyverse) if("comments" %in% colnames(df)) { df$weight <- as.numeric(df$weight) df <- df %>% pivot_wider(names_from = session, values_from=c(weight, active, inactive, reward, beam_breaks, latency.to.first.active, latency.to.first.inactive, comments)) } else {df <- df %>% pivot_wider(names_from = session, values_from=c(weight, active, inactive, reward, beam_breaks, latency.to.first.active, latency.to.first.inactive))} # Change class of columns and remove NA values df <- as.data.frame(df) df <- sapply(df, as.character) df[df == "#N/A"] <- NA df[is.na(df)] <- " " # df <- df[, colSums(df != 0) > 0] # Write csv file to the working directory write.csv(df,file=paste(funcpath,SA_rearrangement_csvname,sep=""), row.names=FALSE) # write.csv(df, "SA_rearrangement_output.csv", row.names = FALSE)
...
# sensitive value redacted # # LOADING LIBRARIES ----
# library(plyr)
# library(lubridate)
# library(DT)
# library(lme4)
